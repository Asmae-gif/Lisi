...
import { ProjectSettings, ApiResponse, Section, DEFAULT_PROJECT_SETTINGS } from '@/types/ProjectSettings';
import { buildImageUrl } from '@/utils/imageUtils';

export default function SettingsProjets() {
  // Définition des sections du formulaire
  const sections: Section[] = useMemo(() => [
    {
      title: "Section Hero - Français",
      fields: [
        { key: "projet_titre_fr", label: "Titre Principal (Français)", type: "text" },
        { key: "projet_sous_titre_fr", label: "Sous-titre (Français)", type: "text" },
      ],
    },
    {
      title: "Section Hero - English",
      fields: [
        { key: "projet_titre_en", label: "Main Title (English)", type: "text" },
        { key: "projet_sous_titre_en", label: "Subtitle (English)", type: "text" },
      ],
    },
    {
      title: "Section Hero - العربية",
      fields: [
        { key: "projet_titre_ar", label: "العنوان الرئيسي (العربية)", type: "text" },
        { key: "projet_sous_titre_ar", label: "العنوان الفرعي (العربية)", type: "text" },
      ],
    },
    {
  ...
  const [values, setValues] = useState<ProjectSettings>(() => {
  ...
  const loadSettings = useCallback(async () => {
    try {
      ...
      if (settingsData && typeof settingsData === 'object') {
        const defaultValues: ProjectSettings = {
  ...
  const handleChange = useCallback((key: string, e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    if (e.target.type === 'file') {
      ...
      if (file) {
        ...
        if (values[key as keyof ProjectSettings]) {
          const imageUrl = buildImageUrl(String(values[key as keyof ProjectSettings]))
          if (imageUrl) {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append('page', 'projects'); // Changed from 'publications' to 'projects'
      ...
      const response = await axiosClient.post<ApiResponse>(
        '/api/pages/projects/settings', // Changed from 'publications' to 'projects'
        formData,
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      <h1 className="text-2xl font-bold">Paramètres — Page Projets</h1>
      <SettingsForm
        sections={sections}
